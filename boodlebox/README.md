...
The bot now defaults to a website tour. It opens the homepage before asking for a choice, uses Russian thistle to teach search, photographs and habitat information, then Showy milkweed to teach pet warnings and source scope, before the practice-note handoff. No actual specimen or prior plant knowledge is required. A note marked Tutorial practice is discussed as a learning exercise, not as a specimen identification. Returning visitors can skip the tour. `greeting.md` and `description.md` are the separately published communication fields; `instructions.md` controls the conversation.
