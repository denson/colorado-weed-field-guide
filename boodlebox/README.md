...
The site builds static per-plant reference packets from the same Markdown as the human articles. A visitor selects zero, one or two profiles and prepares a note with observations plus article/evidence text. No observation backend, API key, local storage, automatic submission or third-party page script is used. Selecting a profile does not identify a specimen.

`instructions.md` is the bot configuration source. `colorado-weed-guide-reference.md` is the uploaded dated knowledge snapshot. After updating profiles: run the normal revision/build/validation workflow, then `python boodlebox/build_reference.py` and replace the bot attachment if desired. Prepared field notes already carry the newly deployed text; the installed extension does not need an update for plant content changes.
